...
def setup_logging():
    ...
    with open(log_path, "w", encoding="utf-8") as log:
        log.write(f"=== Image Transfer Log: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
    
    return log_path

# === GLOBALS ===
LOG_FILE = setup_logging()  # Initialize with a unique log file path
DESTINATION_ROOT = r"\\10.7.8.8\cor\PRD\__Product_Quality"  # Root path where images will be organized and moved
...
def main():
    ...
    - Validates filenames and moves images
    ...
    source_folder = os.path.join(base_path, "Images")

    # Create 'Images' directory if it doesn't exist
    if not os.path.exists(source_folder):
        try:
            os.makedirs(source_folder)
            log_message(f"'Images' folder created at: {source_folder}\n", print_console=True)
        except OSError as e:
            log_message(f"Error creating 'Images' folder: {e}", print_console=True)
            sys.exit(1)
    ...
    log_message("Image Transfer Script execution started", print_console=True)
    log_message("This version automatically determines destination folders from filenames.\n", print_console=True)

    # Start timing
    start_time = time.time()
    total_images_processed = 0
    destination_counts = {}  # Track images by destination
    prefix_counts = {}       # Track images by prefix
    ...
    average_time = (end_time - start_time) / total_images_processed if total_images_processed > 0 else 0

    # === FINAL SUMMARY ===
    log_message("=" * 40, print_console=True)
    log_message(f"Total images processed: {total_images_processed}", print_console=True)
    log_message(f"Total images moved: {moved_count}", print_console=True)
    log_message(f"Total images skipped: {skipped_count}", print_console=True)    
    log_message(f"Total time taken: {minutes} minute(s) {seconds} second(s)", print_console=True)
    log_message(f"Average time per image: {average_time:.2f} second(s)", print_console=True)
    log_message("=" * 40, print_console=True)

    # Display destination breakdown
    if destination_counts:
        log_message("Images by destination:", print_console=True)
        for dest, count in destination_counts.items():
            log_message(f"  {dest}: {count} image(s)", print_console=True)
    log_message("=" * 40, print_console=True)
    
    # Display prefix breakdown
    if prefix_counts:
        log_message("Images by prefix:", print_console=True)
        for prefix, count in prefix_counts.items():
            log_message(f"  {prefix}: {count} image(s)", print_console=True)
    log_message("=" * 40, print_console=True)
    
    log_message("Image Transfer Script execution completed.", print_console=True)
    log_message("*" * 80, print_console=True)
